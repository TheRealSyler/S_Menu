        ...
        if tabs.SM_RS_cycles_tabs == 'OUTPUT':
            ...
            if tabs.SM_RS_output_sub_tabs == 'OUTPUT':
                ...
                if scene.auto_save_after_render is True:
                    col = box.column(align=True)
                    col.prop(context.scene, 'auto_save_format',
                             text='as', expand=False)
                    col.prop(context.scene, 'auto_save_blend', toggle=False)
                    col.prop(context.scene,
                             'auto_save_subfolders', toggle=False)
                    col.prop(context.scene,
                             'auto_save_use_framenumber', toggle=False)
        ...
        if tabs.SM_RS_eevee_tabs == 'PP':
            ...
            if tabs.SM_RS_eevee_sub_tabs == 'VOLUMETRIC':
                if props.use_volumetric is True:
                    ...
                    box = layout.box()
                    box.prop(props, "use_volumetric_lights",
                             text="Volumetric Lighting")
                    if props.use_volumetric_lights is True:
                        box.prop(props, "volumetric_light_clamp",
                                 text="Light Clamping")
                    box = layout.box()
                    box.prop(props, "use_volumetric_shadows",
                             text="Volumetric Shadows")
                    if props.use_volumetric_shadows is True:
                        box.prop(props, "volumetric_shadow_samples",
                                 text="Shadow Samples")
                else:
    ...
    if gaf is True:
        ...
        if tabs == "LIGHTS":
            if context.scene.render.engine == "CYCLES":
                layout.prop(gaf_props, 'hdri_handler_enabled',
...
class SM_OT_Main_Popup(bpy.types.Operator):
    ...
    def _draw_framerate_label(*args):
        ...
        custom_framerate = (
            fps_rate not in {23.98, 24, 25, 29.97, 30, 50, 59.94, 60})
        ...
        RENDER_PT_dimensions._frame_rate_ret = args = (
    ...
    def draw_framerate(layout, sub, rd):
        ...
        fps_label_text, show_framerate = RENDER_PT_dimensions._draw_framerate_label(
            *args)
    ...
    def draw(self, context):
        ...
        if self.single_panel is True:
            box = layout.box()
            box.prop(self, "single_panel", text="Go To Main Panel",
                     icon="DECORATE", expand=True)
