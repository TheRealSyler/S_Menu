...
from . ui.change_workspaces_pie import SM_MT_pie_workspaces_menu_Call
from . ui.get_icon import get_icon
from . ui.main_popup_panel import SM_OT_Main_Popup
...
def add_hotkey():
    ...
    kmi = km.keymap_items.new(
        SM_MT_pie_add_call.bl_idname, 'A', 'PRESS', ctrl=False, shift=True)
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new(
        SM_MT_pie_q_menu_call.bl_idname, 'Q', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new(
        SM_MT_pie_a_om_call.bl_idname, 'A', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
    ...
    kmi = km.keymap_items.new(
        SM_MT_pie_tab_menu_call.bl_idname, 'TAB', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new(
        SM_MT_pie_m4_menu_call.bl_idname, 'BUTTON4MOUSE', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))

    #
    km = kc.keymaps.new(name='Window')
    kmi = km.keymap_items.new(
        SM_MT_pie_w_menu_call.bl_idname, 'W', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new(
        SM_MT_pie_workspaces_menu_Call.bl_idname, 'W', 'PRESS', ctrl=True, shift=False)
    addon_keymaps.append((km, kmi))
    kmi = km.keymap_items.new(
        SM_OT_Main_Popup.bl_idname, 'X', 'PRESS', ctrl=True, shift=True)
    ...
    kmi = km.keymap_items.new(
        SM_MT_ot_w_sculpt_menu_call.bl_idname, 'W', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
    ...
    kmi = km.keymap_items.new(
        SM_MT_pie_add_node_call.bl_idname, 'A', 'PRESS', ctrl=False, shift=True)
    addon_keymaps.append((km, kmi))

    kmi = km.keymap_items.new(
        SM_MT_pie_q_node_call.bl_idname, 'Q', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))

    kmi = km.keymap_items.new(
        SM_MT_pie_a_node_call.bl_idname, 'A', 'PRESS', ctrl=False, shift=False)
    addon_keymaps.append((km, kmi))
...
class SM_Prefs(bpy.types.AddonPreferences):
    ...
    options_sub_tabs: EnumProperty(
        name="Options_sub_Tab", items=options_sub_tabs)
    ...
    # $ Workspaces options
    custom_workspace_pie: BoolProperty(
        name="Customize Workspace Pie", default=False)
    ...
    show_delete_instances: BoolProperty(
        name="Delete All Instances", default=False)
    sm_mh_del_inst = [
        ("NO", "No", ""),
        ("YES", "Yes", ""),
    ]
    ...
    def add_keymap_to_ui(self, context, layout, k_name, idname):
        ...
        row.prop(keymap_item[idname], 'type',
                 text=keymap_item[idname].name, full_event=True)
    ...
    def List_tab(self, context, col):
        ...
        if self.collapse_list_menus is False:
            ...
            if self.collapse_list_object_mode is False:
                self.add_keymap_to_ui(
                    context, sub, 'Object Mode', SM_MT_pie_add_call.bl_idname)
                self.add_keymap_to_ui(
                    context, sub, 'Object Mode', SM_MT_pie_a_om_call.bl_idname)
                self.add_keymap_to_ui(
                    context, sub, 'Object Mode', SM_MT_pie_q_menu_call.bl_idname)
            ...
            if self.collapse_list_view3d is False:
                self.add_keymap_to_ui(
                    context, sub, 'Object Non-modal', SM_MT_pie_tab_menu_call.bl_idname)
                self.add_keymap_to_ui(
                    context, sub, 'Object Non-modal', SM_MT_pie_m4_menu_call.bl_idname)
            ...
            if self.collapse_list_window is False:
                self.add_keymap_to_ui(
                    context, sub, 'Window', SM_MT_pie_w_menu_call.bl_idname)
                self.add_keymap_to_ui(
                    context, sub, 'Window', SM_MT_pie_workspaces_menu_Call.bl_idname)
                self.add_keymap_to_ui(
                    context, sub, 'Window', SM_OT_Main_Popup.bl_idname)
            ...
            if self.collapse_list_sculpt is False:
                self.add_keymap_to_ui(
                    context, sub, 'Sculpt', SM_MT_ot_w_sculpt_menu_call.bl_idname)
            ...
            if self.collapse_list_node is False:
                self.add_keymap_to_ui(
                    context, sub, 'Node Generic', SM_MT_pie_add_node_call.bl_idname)
                self.add_keymap_to_ui(
                    context, sub, 'Node Generic', SM_MT_pie_q_node_call.bl_idname)
                self.add_keymap_to_ui(
                    context, sub, 'Node Generic', SM_MT_pie_a_node_call.bl_idname)
    ...
    def Options_sub_tab_main(self, context, col):
        ...
        if self.collapse_list_options is False:
            ...
            sub.prop(self, "enable_pose_buttons",
                     text="Enable Copy/Paste Buttons In Header")
            sub.prop(self, "enable_debug_messages",
    ...
    def utils_main_tab(self, context, col):
        ...
        self.add_keymap_to_ui(context, col, 'Object Mode',
                              SM_MT_pie_a_om_call.bl_idname)
        self.add_keymap_to_ui(context, col, 'Node Generic',
                              SM_MT_pie_a_node_call.bl_idname)
        self.add_keymap_to_ui(context, col, 'Object Non-modal',
                              SM_MT_pie_tab_menu_call.bl_idname)
        self.add_keymap_to_ui(
            context, col, 'Object Non-modal', SM_MT_pie_m4_menu_call.bl_idname)
    ...
    def add_sub_object(self, context, col):
        ...
        col.prop(self, "enable_extra_objects_mesh",
                 text="Extra Objects (Mesh)")
        col.prop(self, "enable_images_as_planes",
                 text="Import Images as Planes")
        col.prop(self, "enable_camera_rigs", text="Add Camera Rigs")

        col.label(text="Keymap:")
        self.add_keymap_to_ui(context, col, 'Object Mode',
                              SM_MT_pie_add_call.bl_idname)

    def add_sub_node(self, context, col):
        col.label(text="Keymap:")

        self.add_keymap_to_ui(context, col, 'Node Generic',
                              SM_MT_pie_add_node_call.bl_idname)
    ...
    def q_sub_object(self, context, col):
        ...
        self.add_keymap_to_ui(context, col, 'Object Mode',
                              SM_MT_pie_q_menu_call.bl_idname)

    def q_sub_node(self, context, col):
        col.label(text="Options:")
        col.label(text="Keymap:")
        self.add_keymap_to_ui(context, col, 'Node Generic',
                              SM_MT_pie_q_node_call.bl_idname)
    ...
    def workspaces_options_tab(self, context, col):
        ...
        col.label(text="Keymap:")
        self.add_keymap_to_ui(context, col, 'Window',
                              SM_MT_pie_workspaces_menu_Call.bl_idname)
        ...
        self.workspace_pie_slot_options = self.get_workspaces()

        box.label(text="Available Workspaces:")

        box.prop(self, "workspace_pie_slot_options")
        box.prop(self, "workspace_pie_slot_1",
                 icon_value=get_icon("pie_01_icon", "main"))
        box.prop(self, "workspace_pie_slot_1_icon", icon="HIDE_OFF")
        box.prop(self, "workspace_pie_slot_2",
                 icon_value=get_icon("pie_02_icon", "main"))
        box.prop(self, "workspace_pie_slot_2_icon", icon="HIDE_OFF")
        box.prop(self, "workspace_pie_slot_3",
                 icon_value=get_icon("pie_03_icon", "main"))
        box.prop(self, "workspace_pie_slot_3_icon", icon="HIDE_OFF")
        box.prop(self, "workspace_pie_slot_4",
                 icon_value=get_icon("pie_04_icon", "main"))
        box.prop(self, "workspace_pie_slot_4_icon", icon="HIDE_OFF")
        box.prop(self, "workspace_pie_slot_5",
                 icon_value=get_icon("pie_05_icon", "main"))
        box.prop(self, "workspace_pie_slot_5_icon", icon="HIDE_OFF")
        box.prop(self, "workspace_pie_slot_6",
                 icon_value=get_icon("pie_06_icon", "main"))
        box.prop(self, "workspace_pie_slot_6_icon", icon="HIDE_OFF")
        box.prop(self, "workspace_pie_slot_7",
                 icon_value=get_icon("pie_07_icon", "main"))
        box.prop(self, "workspace_pie_slot_7_icon", icon="HIDE_OFF")
        box.prop(self, "workspace_pie_slot_8",
                 icon_value=get_icon("pie_08_icon", "main"))
